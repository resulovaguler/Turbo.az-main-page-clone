...
for (let i = 0; i < data.length; i++) {
  let item = data[i]
  kod += `<div class="max-w-[220px] w-[200px] rounded-[7px] shadow-md dark:bg-gray-50 dark:text-gray-800">
              <div class="relative">
                  <svg class="absolute top-[10px] right-[10px]" xmlns="http://www.w3.org/2000/svg" width="30" height="27" viewBox="-5 -5 30 27" x="675" y="58"><path d="M14.35.766a4.89 4.89 0 014.212 2.373 4.715 4.715 0 01.432 3.906c-.83 3.651-7.302 7.975-8.993 9.052-1.692-1.077-8.168-5.4-8.975-8.978a4.86 4.86 0 01-.26-1.549c0-.888.245-1.719.672-2.432a4.877 4.877 0 011.848-1.772 4.94 4.94 0 012.362-.6 4.917 4.917 0 014.356 2.615 4.88 4.88 0 01.584-.872A4.901 4.901 0 0114.349.766z" fill="#212C3A" fill-rule="nonzero" stroke="#FFF" stroke-width="1.531" fill-opacity=".297"/></svg>
                  <img src="${item.images}" alt="" class="object-cover object-center w-full rounded-t-md min-h-[109px] h-[176px] dark:bg-gray-500">
                  <div class="bg-[#fff] inline-flex justify-center items-center p-[2px] rounded-[4px] absolute bottom-[10px] right-[10px]">
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="18" fill="none" viewBox="-5 -5 26 23" x="648" y="158"><path d="M11.06 5.2L8 0 4.94 5.2.5 2.6l1.875 7.15V13h11.25V9.75L15.5 2.6l-4.44 2.6z" fill="#FF9F2B"/></svg>
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="18" fill="none" viewBox="-5 -5 26 23" x="648" y="181"><path fill-rule="evenodd" clip-rule="evenodd" d="M11.75 0h-7.5L.5 4.643 8 13l7.5-8.357L11.75 0zM7.39 8.03L5.374 2.677l1.535-.002 1.128 3.96 1.175-3.96 1.413.002-2.02 5.35-1.215.001z" fill="#FF3D00"/></svg>
                  </div>
              </div>
              <div class="flex flex-col justify-between p-[10px]">
                  <p class="mb-[2px] whitespace-nowrap font-bold text-[15px]">${item.price} ${item.currency}</p>
                  <p class="whitespace-nowrap text-[14px] font-[500]">${item.brand} ${item.model}</p>
                  <span class="whitespace-nowrap mb-[2px] text-[14px]">${item.year}, ${item.engine}L, ${item.odometer}${item.odometerUnit}</span>
                  <span class="text-[#8d94ad] text-[12px]">${item.city}, bugün 16:50</span>
              </div>
          </div>`
}
...
for (let i = 0; i < 15; i++) {
  let item = data[i]
  firstt += `<li>${item.brand}</li> </br>`
}
first.innerHTML = firstt

let sec = ''
for (let i = 15; i < 30; i++) {
  let item = data[i]
  sec += `<li>${item.brand}</li> </br>`
}
